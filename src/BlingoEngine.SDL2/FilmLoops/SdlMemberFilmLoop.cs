...
namespace BlingoEngine.SDL2.FilmLoops;

/// <summary>
/// SDL implementation of a film loop member. It composes the layers of a
/// <see cref="BlingoFilmLoopMember"/> into an SDL texture for rendering.
/// </summary>
public class SdlMemberFilmLoop : IBlingoFrameworkMemberFilmLoop, IDisposable
{
    ISdlRootComponentContext _sdlRootContext;
    private BlingoFilmLoopMember _member = null!;
    public bool IsLoaded { get; private set; }
    public byte[]? Media { get; set; }
    public IAbstTexture2D? TextureBlingo { get; private set; }
    ...
    public void Unload()
    {
        if (TextureBlingo is SdlTexture2D tex && tex.Handle != nint.Zero)
        {
    ...
    public IAbstTexture2D? RenderToTexture(BlingoInkType ink, AColor transparentColor) => TextureBlingo;
    /// <summary>
    /// Composes the provided layers into a single SDL texture using the
    /// <see cref="BlingoFilmLoopComposer"/> for layout and transforms.
    /// </summary>
    ...
    public IAbstTexture2D ComposeTexture(IBlingoSprite2DLight hostSprite, IReadOnlyList<BlingoSprite2DVirtual> layers, int frame)
    {
        ...
        if (surface == nint.Zero)
            return TextureBlingo ?? new SdlTexture2D(nint.Zero, width, height);
        var i = 0;
        foreach (var info in prep.Layers)
        {
            ...
            int widthS = 0;
            int heightS = 0;
            if (info.Sprite2D.Member is BlingoMemberBitmap pic && pic.FrameworkObj is SdlMemberBitmap bmp)
            {
                textureSdl = ((IBlingoFrameworkMemberBitmap)bmp).TextureBlingo as SdlTexture2D;
        ...
        TextureBlingo = new SdlTexture2D(texture, width, height, $"Filmloop_{frame}_Member_" + _member.Name);
        return TextureBlingo;
    }
#if DEBUG

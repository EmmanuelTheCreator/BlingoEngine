...
{
    private static readonly string[] _projectorMarkers = { "PJ93", "PJ95", "PJ00", "PJ01" };
    ...
    {
        foreach (var marker in _projectorMarkers)
        {
    ...
    private static bool IsMagic(ReadOnlySpan<byte> bytes) 
        => BlTag.Equals(bytes, 0, BlTag.RIFX) || BlTag.Equals(bytes, 0, BlTag.XFIR);

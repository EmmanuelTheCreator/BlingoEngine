...
namespace BlingoEngine.Tools
{
    public class CsvImporter
    {
        ...
        public record CsvRow
        {
            ...
            public BlingoMemberType Type { get; set; }
            public string Name { get; set; }
            public APoint RegPoint { get; set; }
            public string FileName { get; set; }

            public CsvRow(int number, BlingoMemberType type, string name, APoint registration, string fileName)
            {
                Number = number;
                Type = type;
                Name = name;
        ...
        ///     1,bitmap,BallB,"(5, 5)",
        /// </summary>
        public void ImportInCastFromCsvFile(IBlingoCast cast, string filePath, bool skipFirstLine = true, Action<string>? logWarningMethod = null) =>
            ImportInCastFromCsvFileAsync(cast, filePath, skipFirstLine, logWarningMethod).GetAwaiter().GetResult();

        public async Task ImportInCastFromCsvFileAsync(IBlingoCast cast, string filePath, bool skipFirstLine = true, Action<string>? logWarningMethod = null)
        {
            ...
            foreach (var row in csv)
            {
                ...
                if (string.IsNullOrWhiteSpace(row.FileName))
                {
                    ...
                    switch (row.Type)
                    {
                        case BlingoMemberType.Text:
                            ext = ".txt";
                            break;
                        case BlingoMemberType.Field:
                            ext = ".txt";
                            break;
                        case BlingoMemberType.Sound:
                            ext = ".wav";
                            break;
                        case BlingoMemberType.Script:
                            ext = ".cs";
                            break;
                    }
                ...
                if (newMember is IBlingoMemberTextBase textMember)
                {
        ...
        public async Task<IReadOnlyCollection<CsvRow>> ImportCsvCastFileAsync(string filePath, bool skipFirstLine = true)
        {
            ...
            foreach (var row in csv)
            {
                ...
                var fileName = row[4];
                var type1 = BlingoMemberType.Unknown;

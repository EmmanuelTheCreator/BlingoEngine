...
/// </summary>
public static class BlingoFilmLoopComposer
{
    /// <summary>
    ...
    public readonly record struct LayerInfo(
        //IBlingoFrameworkMemberBitmap Bitmap,
        //IBlingoImageTexture Texture,
        int SrcX,
        int SrcY,
        int SrcW,
        int SrcH,
        ...
        float Alpha,
        BlingoInkType Ink,
        AColor BackColor, BlingoSprite2DVirtual Sprite2D);
    ...
    public static (APoint Offset, int Width, int Height, List<LayerInfo> Layers) Prepare(BlingoFilmLoopMember filmLoop, BlingoFilmLoopFraming framing, IEnumerable<BlingoSprite2DVirtual> layers)
    {
        ...
        foreach (var layer in layers)
        {
            int srcW = 0;
            int srcH = 0;
            if (layer.Member is IBlingoMemberWithTexture)
            {
            ...
            if (layer.Member is BlingoFilmLoopMember nested)
            {
                var fl = nested.Framework<IBlingoFrameworkMemberFilmLoop>();
                pos.X -= fl.Offset.X;
                pos.Y -= fl.Offset.Y;
            }
...
#if DEBUG
    private static void DebugInfo(IEnumerable<BlingoSprite2DVirtual> layers, ARect bounds, APoint offset, int width, int height)
    {
        Console.WriteLine("Running framework: " + BlingoEngineGlobal.RunFramework);

...
{
    public class BlingoSoundDevice
    {
        public int Index { get; }
        public string Name{ get; }
        public BlingoSoundDevice(int i, string name)
        {
            Index = i;
            Name = name;
        }
    ...
    public interface IBlingoSound
    {
        ...
        /// </summary>
        BlingoSoundDevice SoundDevice { get; set; }
        ...
        BlingoSoundChannel? Channel(int channelNumber);
        ...
        void PuppetSound(int channelNumber, BlingoMemberSound member, float startTime = -1, float endTime = -1, int loopCount = -1, float loopStartTime = -1, float loopEndTime = -1, float preloadTime = -1);
        void StopAll();
    }


    /// <inheritdoc/>
    public class BlingoSound : IBlingoSound
    {
        private Dictionary<int, BlingoSoundChannel> _Channels = new();
        private int numberOfSoundChannels = 8;
        private readonly IBlingoCastLibsContainer _castLibsContainer;
        private readonly IBlingoFrameworkSound _frameworkSound;
        public T FrameworkObj<T>() where T : IBlingoFrameworkSound => (T)_frameworkSound;

        /// <inheritdoc/>
        public BlingoSoundDevice SoundDevice { get; set; } = new BlingoSoundDevice(0,"Default");

        /// <inheritdoc/>
        public List<BlingoSoundDevice> SoundDeviceList { get => _frameworkSound.SoundDeviceList; }
        ...
        /// <inheritdoc/>
        public BlingoSound(IBlingoFrameworkSound frameworkSound, IBlingoCastLibsContainer castLibsContainer, IBlingoFrameworkFactory factory)
        {
        ...
        {
            var member = _castLibsContainer.GetMember<BlingoMemberSound>(memberName);
            if (member == null) return;
            _Channels[channelNumber].Play(member, startTime,endTime, loopCount, loopStartTime, loopEndTime,preloadTime);
        }
        public void PuppetSound(int channelNumber, BlingoMemberSound member, float startTime = -1, float endTime = -1, int loopCount = -1, float loopStartTime = -1, float loopEndTime = -1, float preloadTime = -1)
        {
            if (member == null) return;
            _Channels[channelNumber].Play(member, startTime,endTime, loopCount, loopStartTime, loopEndTime,preloadTime);
        }
    ...
    /// </summary>
    public enum BlingoSoundChannelStatus
    {
        /// <summary>

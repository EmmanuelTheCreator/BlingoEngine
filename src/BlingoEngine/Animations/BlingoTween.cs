...
{
    public class BlingoTween<T>
    {
        private readonly List<BlingoKeyFrame<T>> _keys = new();
        private bool _hasFirstKeyFrame;

        public IReadOnlyList<BlingoKeyFrame<T>> KeyFrames => _keys;
        public BlingoTweenOptions Options { get; private set; } = new();

        public void AddKeyFrame(int frame, T value, BlingoEaseType ease = BlingoEaseType.Linear)
        {
            if (frame == 1) _hasFirstKeyFrame = true;
            var k = new BlingoKeyFrame<T>(frame, value) { Ease = ease };
            _keys.Add(k);
            _keys.Sort((a, b) => a.Frame.CompareTo(b.Frame));
        }

        public void UpdateKeyFrame(int frame, T value, BlingoEaseType ease = BlingoEaseType.Linear)
        {
        ...
        private static float ApplyEase(float t, BlingoEaseType ease)
        {
            return ease switch
            {
                BlingoEaseType.EaseIn => t * t,
                BlingoEaseType.EaseOut => t * (2 - t),
                BlingoEaseType.EaseInOut => t < 0.5f ? 2 * t * t : -1 + (4 - 2 * t) * t,
                _ => t,
            };
        }

        private static float ApplySpeedChange(float t, BlingoSpeedChangeType type)
        {
            return type == BlingoSpeedChangeType.Smooth ? t * t * (3 - 2 * t) : t;
        }
        ...
        internal BlingoTween<T> Clone()
        {
            var clone = new BlingoTween<T>();

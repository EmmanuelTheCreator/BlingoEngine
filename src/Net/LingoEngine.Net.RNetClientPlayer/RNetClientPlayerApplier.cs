...
internal sealed class RNetClientPlayerApplier
{
    ...
    public RNetClientPlayerApplier(ILingoPlayer player)
    {
        _player = player;
    }
    ...
    {
        if (GetSprite(keyframe.SpriteNum, keyframe.BeginFrame) is not LingoSprite2D sprite)
        {
            return;
        }
    ...
    private static bool TryBuildSetting(LingoSprite2D sprite, KeyframeDto dto, out LingoKeyFrameSetting setting)
    {
        setting = new LingoKeyFrameSetting(dto.Frame);

        switch (dto.Prop)
        {
            case nameof(LingoSprite2D.LocH):
                ...
                break;
            case nameof(LingoSprite2D.LocV):
                ...
                break;
            case nameof(LingoSprite2D.Width):
                ...
                break;
            case nameof(LingoSprite2D.Height):
                ...
                break;
            case nameof(LingoSprite2D.Rotation):
                ...
                break;
            case nameof(LingoSprite2D.Blend):
                ...
                break;
            case nameof(LingoSprite2D.Skew):
    ...
    {
        var member = _player.CastLibs.GetMember<LingoFilmLoopMember>(loop.MemberNum, loop.CastLibNum);
        if (member == null)
        {
            return;
        }
    ...
    public void ApplyTransition(TransitionDto transition)
    {
        if (_player.ActiveMovie is LingoMovie movie)
        {
            var settings = new LingoTransitionFrameSettings { TransitionName = transition.Type };
    ...
    {
        if (_player.ActiveMovie is not LingoMovie movie || evt.Manager != "Sprite2D")
        {
            return;
        }
        var manager = movie.Sprite2DManager;
    ...
    {
        if (_player.CastLibs.GetMember(style.MemberNum, style.CastLibNum) is ILingoMemberTextBase text)
        {
            RNetPropertySetter.TrySet(text, style.Style, style.Value);
        }
    }

    private LingoSprite2D? GetSprite(int spriteNum, int beginFrame)
    {
        if (_player.ActiveMovie is not LingoMovie movie)
        {
            return null;
    ...
    private void ApplySpriteState(LingoSprite2D sprite, RNetSpriteDto dto)
    {
        sprite.LocZ = dto.Z;
        if (dto.MemberNum != 0)
        {

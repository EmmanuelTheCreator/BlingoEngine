...
namespace BlingoEngine.Unity.FilmLoops;

/// <summary>
/// Unity implementation of a film loop member. It composes sprite layers into
/// a single <see cref="Texture2D"/> using the <see cref="BlingoFilmLoopComposer"/>.
/// </summary>
public class UnityFilmLoopMember : IBlingoFrameworkMemberFilmLoop, IDisposable
{
    ...
    public IAbstTexture2D ComposeTexture(IBlingoSprite2DLight hostSprite, IReadOnlyList<BlingoSprite2DVirtual> layers, int frame)
    {
        ...
        foreach (var info in prep.Layers)
        {
            ...
            if (info.Sprite2D.Member is IBlingoMemberWithTexture m)
            {

...
namespace BlingoEngine.Director.SDL2.Icons
{
    public class BlingoIconSheetSdl : BlingoIconSheet<SdlTexture2D>
    {
        public BlingoIconSheetSdl(SdlTexture2D image, int iconWidth, int iconHeight, int horizontalSpacing, int iconCount)
            : base(image, iconWidth, iconHeight, horizontalSpacing, iconCount)
        {
        }
    }

    public class DirSdlIconManager : DirectorIconManager<BlingoIconSheetSdl>
    {
        private readonly ILogger _logger;
        private readonly nint _renderer;

        public DirSdlIconManager(ILogger<DirSdlIconManager> logger, BlingoSdlRootContext context)
        {
            _logger = logger;
            _renderer = context.Renderer;
        }

        protected override BlingoIconSheetSdl? OnLoadSheet(string path, int itemCount, int iconWidth, int iconHeight, int horizontalSpacing = 0)
        {

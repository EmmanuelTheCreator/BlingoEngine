...
internal partial class DirScoreColorPaletteGridChannel : DirScoreChannel<IBlingoSpriteColorPaletteSpriteManager, DirScoreColorPaletteSprite, BlingoColorPaletteSprite>
{
    private readonly IBlingoColorPaletteDefinitions _paletteDefinitions;
    
    private BlingoColorPaletteFrameSettings? _currentSettings;
    ...
    public DirScoreColorPaletteGridChannel(IDirScoreManager scoreManager, IBlingoColorPaletteDefinitions paletteDefinitions)
        : base(BlingoColorPaletteSprite.SpriteNumOffset+1, scoreManager)
    {
        _paletteDefinitions = paletteDefinitions;
        IsSingleFrame = true;
    }

    protected override DirScoreColorPaletteSprite CreateUISprite(BlingoColorPaletteSprite sprite, IDirSpritesManager spritesManager) => new DirScoreColorPaletteSprite(sprite, spritesManager);

    protected override IBlingoSpriteColorPaletteSpriteManager GetManager(BlingoMovie movie) => movie.ColorPalettes;

    internal override void ShowCreateSpriteDialog(int frameNumber, Action<BlingoSprite?> newSprite)
    {
        var settings = new BlingoColorPaletteFrameSettings();
        Action okAction = () =>
        {
    ...
    {
        if (!(sprite is BlingoColorPaletteSprite paletteSprite)) return;
        var settings = paletteSprite.GetSettings();
        if (settings == null)
            settings = new BlingoColorPaletteFrameSettings();
        Action okAction = () =>
        {
            if (settings != null)
    ...
    private void ShowDialog(BlingoColorPaletteFrameSettings settings, Action okAction)
    {
        ...
        {
            if (Enum.TryParse<BlingoColorPaletteAction>(key, out var action))
        ...
        {
            if (Enum.TryParse<BlingoColorPaletteTransitionOption>(key, out var option))
    ...
    private void RedrawColors(AbstGfxCanvas colorsCanvas, BlingoColorPaletteDefinition definition)
    {

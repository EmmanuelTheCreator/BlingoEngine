...
internal partial class DirScoreTransitionGridChannel : DirScoreChannel<IBlingoSpriteTransitionManager, DirScoreTransitionSprite, BlingoTransitionSprite>
{
    ...
    private readonly List<BlingoBaseTransition> _transitions;
    ...
    public DirScoreTransitionGridChannel(IDirScoreManager scoreManager, IBlingoTransitionLibrary transitionLibrary)
        : base(BlingoTransitionSprite.SpriteNumOffset + 1, scoreManager)
    {
    ...
    protected override DirScoreTransitionSprite CreateUISprite(BlingoTransitionSprite sprite, IDirSpritesManager spritesManager)
        => new DirScoreTransitionSprite(sprite, spritesManager);

    protected override IBlingoSpriteTransitionManager GetManager(BlingoMovie movie) => movie.Transitions;

    internal override void ShowCreateSpriteDialog(int frameNumber, Action<BlingoSprite?> newSprite)
    {
        var first = _transitions.First();
        var settings = new BlingoTransitionFrameSettings
        {
    ...
    {
        if (sprite is not BlingoTransitionSprite transitionSprite) return;
        var settings = transitionSprite.GetSettings() ?? new BlingoTransitionFrameSettings();
        Action okAction = () =>
        {
            SetNewValues(settings);
    ...
    private void SetNewValues(BlingoTransitionFrameSettings settings)
    {
    ...
    private void ShowDialog(BlingoTransitionFrameSettings settings, Action okAction)
    {
        ...
        {
            new KeyValuePair<string, string>(BlingoTransitionAffects.EntireStage.ToString(), "Entire Stage"),
            new KeyValuePair<string, string>(BlingoTransitionAffects.ChangingAreaOnly.ToString(), "Changing Area Only"),
            new KeyValuePair<string, string>(BlingoTransitionAffects.Custom.ToString(), "Custom")
        };
        ...
        ToggleRectInputs(settings.Affects == BlingoTransitionAffects.Custom);
        panel.SetComboBoxAt(affectsOptions, "AffectsCombo", 70, 290, 160, settings.Affects.ToString(), key =>
        {
            if (Enum.TryParse<BlingoTransitionAffects>(key, out var affects))
            {
                settings.Affects = affects;
                ToggleRectInputs(affects == BlingoTransitionAffects.Custom);
            }
        });
    ...
    private void PopulateTransitions(BlingoTransitionFrameSettings settings, AbstItemList transitionList, string? category)
    {

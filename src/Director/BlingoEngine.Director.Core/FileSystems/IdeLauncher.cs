...
namespace BlingoEngine.Director.Core.FileSystems
{
    ...
    {
        void OpenFile(BlingoProjectSettings blingoProjectSettings, string filePath, int line = 0);
    ...
    public class IdeLauncher : IIdeLauncher
    {
        ...
        public void OpenFile(BlingoProjectSettings blingoProjectSettings, string filePath, int line = 0)
        {
...
#if USE_WINDOWS_FEATURES
                    OpenInVisualStudio(_directorSettings, blingoProjectSettings, filePath, line);
#else
        ...
        private void OpenInVisualStudio(DirectorProjectSettings settings, BlingoProjectSettings blingoProjectSettings, string filePath, int line)
        {
            string? vsPath = settings.VisualStudioPath;
            if (string.IsNullOrEmpty(vsPath))
                throw new InvalidOperationException("VisualStudioPath must be set.");

            string? slnName = FindSolutionName(blingoProjectSettings.ProjectFolder);

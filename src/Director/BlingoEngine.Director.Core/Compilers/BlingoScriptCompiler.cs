...
namespace BlingoEngine.Director.Core.Compilers;

/// <summary>
/// Compiles project scripts and reloads the active movie.
/// </summary>
public class BlingoScriptCompiler
{
    ...
    private readonly BlingoEngineRegistration _engineRegistration;
    private readonly ILogger<BlingoScriptCompiler> _logger;
    private BlingoPlayer _player;
    private AssemblyLoadContext? _loadContext;
    public BlingoScriptCompiler(
        BlingoPlayer player,
        BlingoProjectSettings settings,
        DirectorProjectSettings dirSettings,
        DirectorProjectManager projectManager,
        IBlingoEngineRegistration engineRegistration,
        ILogger<BlingoScriptCompiler> logger)
    {
        _player = player;
        _settings = settings;
        _dirSettings = dirSettings;
        _projectManager = projectManager;
        _engineRegistration = (BlingoEngineRegistration)engineRegistration;
        _logger = logger;
    }
    ...
    public void Compile()
    {
        var wasPlaying = (_player.ActiveMovie as BlingoMovie)?.IsPlaying == true;
        ...
        if (wasPlaying && _player.ActiveMovie is BlingoMovie movie)
            movie.Play();
    }
    private Assembly? BuildProject()
    {
        ...
        _loadContext = new AssemblyLoadContext("BlingoScripts", isCollectible: true);
    ...
    private Assembly? BuildProject2()
    {
        try
        {
            ...
            parameters.ReferencedAssemblies.Add(typeof(BlingoPlayer).Assembly.Location);
            ...
            _loadContext = new AssemblyLoadContext("BlingoScripts", true);
    ...
    {
        var factoryType = assembly.GetTypes().FirstOrDefault(t => typeof(IBlingoProjectFactory).IsAssignableFrom(t) && !t.IsAbstract && !t.IsInterface);
        if (factoryType == null)
            return;

        var factory = (IBlingoProjectFactory?)Activator.CreateInstance(factoryType);
        if (factory == null)
            return;

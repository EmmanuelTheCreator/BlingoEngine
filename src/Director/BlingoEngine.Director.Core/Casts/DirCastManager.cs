...
public class DirCastManager : IAbstCommandHandler<CreateFilmLoopCommand>
{
    ...
    public bool Handle(CreateFilmLoopCommand command)
    {
        ...
        var cast = (BlingoCast)movie.CastLib.ActiveCast;
        var sprites = command.Sprites.ToList();

        var sprite2D = sprites.OfType<BlingoSprite2D>().ToList();
        var soundSprites = sprites.OfType<BlingoSpriteSound>().ToList();
        ...
        BlingoFilmLoopMember member = cast.Add<BlingoFilmLoopMember>(0, command.Name, fl =>
        {
        foreach (var e in spriteEntries)
            fl.AddSprite(new BlingoFilmLoopMemberSprite(e.Sprite,e.Channel,e.Begin,e.End)); 
        ...
        BlingoFilmLoopMember current = member;
        ...
        Action redo = () =>
        {
            current = cast.Add<BlingoFilmLoopMember>(number, command.Name, fl =>
            {
                foreach (var e in spriteEntries)
                    fl.AddSprite(new BlingoFilmLoopMemberSprite(e.Sprite, e.Channel, e.Begin, e.End));
                foreach (var e in soundEntries)

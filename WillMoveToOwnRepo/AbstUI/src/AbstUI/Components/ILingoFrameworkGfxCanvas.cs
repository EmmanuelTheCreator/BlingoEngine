...
namespace AbstUI.Components
{
    ...
    /// </summary>
    public interface IAbstUIFrameworkGfxCanvas : IAbstUIFrameworkGfxLayoutNode
    {
        ...
        void DrawText(APoint position, string text, string? font = null, AColor? color = null, int fontSize = 12, int width = -1, Texts.AbstUITextAlignment alignment = default);
        void DrawPicture(byte[] data, int width, int height, APoint position, APixelFormat format);
        void DrawPicture(IAbstUITexture2D texture, int width, int height, APoint position);
    }
}

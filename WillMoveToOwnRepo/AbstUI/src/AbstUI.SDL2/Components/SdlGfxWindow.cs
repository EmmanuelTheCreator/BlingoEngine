...
internal class SdlGfxWindow : SdlGfxPanel, IAbstUIFrameworkGfxWindow, IDisposable
{
    ...
    public SdlGfxWindow(AbstUIGfxWindow window, SdlGfxFactory factory) : base(factory)
    {
        _lingoWindow = window;
        _factory = factory;
        var mouse = ((IAbstMouseInternal)factory.RootContext.LingoMouse).CreateNewInstance(window);
        var key = ((AbstKey)factory.RootContext.LingoKey).CreateNewInstance(window);
